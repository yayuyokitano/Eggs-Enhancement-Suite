...
function SettingRadioList(props: {t:TFunction, type: string, names:string[], cur:string, onChange:(name:string) => void}) {
  ...
  return (
    <div className={`${type}-selector`}>
      <h2>{t(`popup.${type}`)}</h2>
      {names.map(name => <SettingRadio key={name} t={t} type={type} name={name} cur={cur} onChange={onChange} />)}
    </div>
  )
}
...
function App() {
  ...
  if (localStorage.getItem("language")) {
	...
	return (
    <div>
      <SettingRadioList t={t} type="language" names={languageList} cur={i18n.resolvedLanguage} onChange={(name:string) => changeLanguage(name, i18n)} />
      <SettingRadioList t={t} type="theme" names={themeList} cur={theme} onChange={setTheme} />
